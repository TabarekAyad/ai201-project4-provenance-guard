...
`GET /log` returns the 20 most recent entries from the `audit_log` table as JSON. Each row is written at classification time and updated in place if an appeal is filed. The three entries below are taken from live testing and show `uncertain` (signals disagree), `likely_human`, and `uncertain` escalated to `under_review` via an appeal.
  ...
  "entries": [
    {
      ...
      "confidence": 0.5257,
      "content_id": "f0ff343c-2a5c-41d5-877f-d0932f282ce4",
      "creator_id": "audit-demo-1",
      "label": "Attribution unclear. Our system found mixed signals for this content and cannot confidently determine whether it was written by a human or generated by AI (confidence: 53%). We've flagged it for transparency, but the creator may contest this classification through an appeal.",
      "llm_score": 0.8,
      "status": "classified",
      "stylometric_score": 0.1904,
      "timestamp": "2026-08-09T22:30:03.145238+00:00"
    },
    {
      "appeal_reasoning": null,
      "appeal_timestamp": null,
      "attribution": "likely_human",
      "confidence": 0.2165,
      "content_id": "e0d666dc-4505-4a5d-a341-09bafcc807a7",
      "creator_id": "audit-demo-2",
      "label": "Appears human-written. Our system found strong indicators that this content was written by a human (confidence: 78% human). No action is required.",
      "llm_score": 0.2,
      "status": "classified",
      "stylometric_score": 0.2366,
      "timestamp": "2026-08-09T22:31:53.461033+00:00"
    },
    {
      "appeal_reasoning": "This is a formal academic writing. This is probably why you are unable to distinguish",
      "appeal_timestamp": "2026-08-09T22:48:33.978492+00:00",
      "attribution": "uncertain",
      "confidence": 0.5365,
      "content_id": "354b33a0-0342-499d-8e44-989117da05ea",
      "creator_id": "audit-demo-3",
      "label": "Attribution unclear. Our system found mixed signals for this content and cannot confidently determine whether it was written by a human or generated by AI (confidence: 54%). We've flagged it for transparency, but the creator may contest this classification through an appeal.",
      "llm_score": 0.8,
      "status": "under_review",
      "stylometric_score": 0.2144,
      "timestamp": "2026-08-09T22:33:04.865362+00:00"
    }
  ]
}
```

Entry 1 (`audit-demo-1`) — AI-style formal prose where the signals disagree: the LLM scores 0.8 but the stylometric signal scores only 0.19, pulling the combined confidence to 0.526, squarely in the uncertain band. The wide uncertain band exists precisely to catch this case rather than asserting a verdict when evidence is mixed.

Entry 2 (`audit-demo-2`) — Casual human text where both signals agree: LLM 0.2, stylometric 0.24, combined confidence 0.217, `likely_human`. The label expresses confidence in the human verdict (78% human) rather than in AI-ness.

Entry 3 (`audit-demo-3`) — Formal academic text initially classified as `uncertain`. The creator filed an appeal disputing the classification, which updated `status` to `under_review` and appended `appeal_reasoning` and `appeal_timestamp` to the same row. No re-classification occurs on appeal — the original scores are preserved for the human reviewer.
